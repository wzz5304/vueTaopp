<template>
	<section class="pr v-slide-content">
		<ul class="v-slide-box pa" ref='vSlide' :class="{'slide-action': true}">
			<li v-for="item in selectVedioList.preview[0].androidPhoneV23Url">
				<img style="height: 90px" :class="{'click-select': lists[0] === item}" :src="`https://gw.alicdn.com/${selectVedioList.poster}`" :data-id="item.id">
			</li>
		</ul>
	</section>
</template>
